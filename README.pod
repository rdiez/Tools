...
Runs the given process with a low priority, keeps a log file and displays a visual notification when finished.
...
Resets file permissions under a given directory on a Windows system.

=item * B<< Office Software Automation >>

=over

=item * B<< PromptAndProcess.vbs >>
...
=item * B<< ConvertWordToPDFWithBackground.vbs >>
...
=item * B<< CopyToOldVersionsArchive.vbs >>
...
Prints the time it takes to run a Windows command.

=item * B<< repeat.bat >>

Cheap clone of I<< watch >> for Windows.
