...
=item * B<< RunAndWaitForAllChildren.pl >>
...
=item * B<< Automount And Run Action >>
...
=item * B<< RunAndReport.sh >> and B<< GenerateHtmlReport.pl >>
...
=item * B<< ReplaceTemplatePlaceholderWithFileContents.sh >> and B<< ReplaceTemplatePlaceholders.sh >>
...
It can also lint CSS with I<< stylelint >>.

=item * B<< Plain Text Linter >>
...
=item * B<< git-stash-only-staged-changes.sh >> and B<< git-stash-only-unstaged-changes.sh >>
