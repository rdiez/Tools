...
First of all, unmount any partitions on the source disk which are mounted as read/write. Then run this command:
...
First of all, unmount any partitions on the destination disk. Then run this command:
...
Afterwards, reload the partition table:

C<< sudo partprobe --summary >>

Tool 'partprobe' is in the Ubuntu/Debian package named 'parted'.

During verification, note that the progress bar will not be very accurate, as I<< pv >> will be measuring
